...
function readSecretFile(path:string){if(resolve(path)!==path)throw Error();for(let p=dirname(path);;){const s=lstatSync(p);if(!s.isDirectory()||s.isSymbolicLink())throw Error();const next=dirname(p);if(next===p)break;p=next;}const a=lstatSync(path);if(a.isSymbolicLink()||!a.isFile()||a.uid!==process.getuid()||(a.mode&0o777)!==0o600)throw Error();const fd=openSync(path,constants.O_RDONLY|((constants as any).O_NOFOLLOW??0));try{const b=fstatSync(fd);if(b.ino!==a.ino||b.dev!==a.dev||b.nlink!==1)throw Error();return JSON.parse(readFileSync(fd,"utf8"));}finally{closeSync(fd)}}
...
export function startAgentEventLoopback(daemon:AgentEventDaemon,config:AgentEventDaemonConfig,port:number,serve:typeof Bun.serve=Bun.serve){if(!Number.isInteger(port)||port<1||port>65535)throw new Error("agent-event port invalid");const ingress=new AgentEventHttpIngress(daemon,config);let server:ReturnType<typeof Bun.serve>;try{server=serve({hostname:"127.0.0.1",port,fetch:r=>ingress.handle(r)} as any);}catch(e){daemon.close();throw e;}let stopped=false;return{port:server.port,async stop(){if(stopped)return;stopped=true;try{await server.stop(true);}finally{daemon.close();}}};}

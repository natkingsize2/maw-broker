...
export class AgentEventHttpIngress{
  ...
  async handle(request:Request):Promise<Response>{
    ...
    const chunks:Uint8Array[]=[];let size=0;const reader=request.body?.getReader();if(!reader)return new Response("invalid",{status:400});for(;;){const{done,value}=await reader.read();if(done)break;size+=value.byteLength;if(size>this.config.maxBodyBytes){await reader.cancel();return new Response("invalid",{status:400});}chunks.push(value);}const raw=new TextDecoder("utf-8",{fatal:true}).decode(Buffer.concat(chunks));
...
export function startAgentEventLoopback(daemon:AgentEventDaemon,config:AgentEventDaemonConfig,port:number){if(!Number.isInteger(port)||port<1||port>65535)throw new Error("agent-event port invalid");const ingress=new AgentEventHttpIngress(daemon,config);const server=Bun.serve({hostname:"127.0.0.1",port,fetch:r=>ingress.handle(r)});let stopped=false;return{port:server.port,async stop(){if(stopped)return;stopped=true;await server.stop(true);daemon.close();}};}
...
export class DiscordMarkerAdapter{constructor(private client:DiscordMarkerClient,private ownActor:string){}async has(destination:string,key:string){let before: string|undefined;const hits:string[]=[];for(let page=0;page<5;page++){const rows=await this.client.listOwnMessages(destination,before);for(const r of rows)if(r.authorId===this.ownActor&&r.webhookId===null&&r.marker===key)hits.push(r.id);if(hits.length>1)throw new Error("Discord marker ambiguous");if(rows.length<50)break;before=rows.at(-1)?.id;}return hits.length===1;}}
...
export class GitHubMarkerAdapter{constructor(private client:GitHubMarkerClient,private ownActor:string){}async has(issueRef:string,key:string){const m=/^([^#]+)#([1-9]\d*)$/.exec(issueRef);if(!m)throw new Error("GitHub issue invalid");const hits:string[]=[];for(let page=1;page<=5;page++){const rows=await this.client.listComments(m[1]!,Number(m[2]),page);for(const r of rows)if(r.actor===this.ownActor&&r.marker===key)hits.push(r.id);if(hits.length>1)throw new Error("GitHub marker ambiguous");if(rows.length<100)break;}return hits.length===1;}}
